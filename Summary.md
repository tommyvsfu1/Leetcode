...
#### 第II類區間型
給一個序列S(數組/字符串)  
* 大區間的最優解，依賴於小區間的最優解 (無法透過單純的後效性建立dp，也就是state i可能取決於state i+1)
* 定義dp[i][j] : 表示s[i:j]的子問題求解
* 千方百計將大區間的dp[i][j]往小區間的dp[i'][j']轉移
* for loop 第一層循環是區間大小；第二層循環是起始點；**有可能要第三層循環k，如果發現拆解state有多種可能(或是一步一步的那種題目)**
* 答案為dp[1][N]

#### 背包型
* 給N件物品，每個物品可用/不可用(或有若干種用法)。要求以某個上限為C的代價來實現最大收益(或某個下限收益來實現最小代價)
* 定義dp[i][c]: 表示考慮只從前i件物品的子集選擇，代價為c的最大收益, c=1,2,...,C
* 千方百計將dp[i][c]以dp[i-1][c']轉移
* 第一層循環是遍歷物品，第二層是遍歷代價的所有可能值
* 最終結果是max{dp[N][c]},c=1,2,...C

```
0/1 背包: dp[i][...] 要從dp[i-1][...]轉移
無限背包: dp[i][...] 要從dp[i][...]轉移
背包+狀態壓縮: 用bitmap代表第二維度[c]，以enumeration bitmap所有可能來for loop c
```
