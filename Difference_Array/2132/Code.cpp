...
public:
    ...
    Prefix2D(vector<vector<int>>& A) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                int a = (i - 1 > 0) ?  presum[i-1][j] : 0;
                int b = (j-1 > 0) ? presum[i][j-1] : 0;
                int c = (i-1 > 0 && j-1 > 0) ? presum[i-1][j-1] : 0;
                presum[i][j] = A[i][j] + a + b - c;
            }
        }
    }
    int query(int i, int j, int x, int y) {
        int a = (i-1 > 0 && j-1 > 0) ? presum[i-1][j-1] : 0;
        int b = (j-1 > 0) ? presum[x][j-1] : 0;
        int c = (i-1 > 0) ? presum[i-1][y] : 0;
